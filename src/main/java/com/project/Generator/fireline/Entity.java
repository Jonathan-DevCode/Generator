...
public class Entity implements StartTask{

	@Override
	public void go(FeedData feedData) {
		try {
			File file = new File(String.format(feedData.getDirectory() + "%sEntity.java", feedData.getFileName()));
			BufferedWriter writer = new BufferedWriter(new FileWriter(file));
			
			String str = String.format(""
			+ "@Entity \r\n"
			+ "@Table(name=\"TABLE_%1$s\")\r\n"
			+ "@Data @Builder @NoArgsConstructor @AllArgsConstructor\r\n"
			+ "public class %1$s implements Serializable {\r\n"
			+ "\r\n"
